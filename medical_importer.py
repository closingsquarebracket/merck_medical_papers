...
def get_keywords(parsed_document):
    list_of_keywords = []
    try:
        kwds = parsed_document.findall('front/article-meta/kwd-group/kwd')
        for kwd in kwds:
            try:
                keyword = kwd.text
                list_of_keywords.append(keyword)
            except AttributeError:
                list_of_keywords.append('n/a')
    except AttributeError:
        list_of_keywords = ['n/a']
...
def clean_index_file():
    ...
    for line in file_lines[1:]:  # leave out initial empty line
        ...
        try:
            _, publication_name, publication_date, *_ = re.split(r"([\w\s()]+.?)\s(\d{4}\s\w{3}\s\d+);",
                                                                 publication)  # :( re is expensive  # 25% unparseable
        except ValueError:
            try:
                _, publication_name, publication_date, *_ = re.split(r"([\w\s()]+.?)\s(\d{4}\s\w{3})[-;\s]",
                                                                     publication)  # :( double re is expensive
                publication_date += ' 1'  # account for missing dates  # down to 2% unparseable
            except ValueError:
                print(line)
                unparseable += 1
                continue
...
if __name__ == '__main__':
    ...
    for block in range(2399000, len(pmcs),
                       citation_block_size):  # in case of crash, restart from last printed 'start' value
        start = block
        print(start)
        ...
        with open('oa_with_citations.txt', 'a') as f_table:
            for counter in range(start, end):
                f_table.write(
                    f"{pmcs[counter]}; {publication_names[counter]}; {publication_dates[counter]}; {n_citations[counter - start]}\n")
